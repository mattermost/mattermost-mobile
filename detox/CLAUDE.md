...
Status contexts live under the `e2e-test/` namespace, matching the mattermost monorepo. Callers pass `context_name`; templates derive the Test System IO report `name` by stripping `e2e-test/` and swapping `/` for `-`. PR: `e2e-test/detox-ios` (etc.). Merge to main: `…/main`. Merge to release: `…/release`. Release cut: `…/release-cut`. CMT: `e2e-test/detox-ios/cmt-server-${version}` (etc.) plus umbrella `e2e-test/compatibility-matrix-testing`. Commit status + channel notify come from `dispatch-begin` / `summary` (webhook_payload curl).
