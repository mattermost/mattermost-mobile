...
import {required, testUserPassword, writeMaestroEnvFile} from '../lib/env';
...
function isRetriableSeedError(message: string): boolean {
    return /cloud\/inactive|HTTP 403|status code 403|Failed to parse|ECONNRESET|ETIMEDOUT|ECONNREFUSED|ENOTFOUND|socket|network|<!DOCTYPE|<html/i.test(message);
}
...
// this, all detox/maestro/flows/calls/* flows hang waiting on calls.current_call_bar.
...
async function main(): Promise<void> {
    ...
    writeMaestroEnvFile(envVars, 'detox/maestro/fixtures/seed.ts');
