...
# Resolve detox/ regardless of caller cwd so npx tsx finds detox/package.json deps.
...
if ! [[ "$SYNC_TOKEN" =~ ^[a-z0-9]+$ ]]; then
  echo "Error: SYNC_TOKEN must contain only lowercase letters and digits" >&2
  exit 1
fi
...
~/.maestro/bin/maestro \
  ...
  maestro/flows/multi_device/device_a_creates_channel.yml \
...
# poll_for_channel.ts already retries up to MAX_POLLS; invoke once (do not nest loops).
if ! SYNC_TOKEN="${SYNC_TOKEN}" TEST_TEAM_ID="${TEST_TEAM_ID}" ADMIN_TOKEN="${ADMIN_TOKEN}" SITE_1_URL="${SITE_1_URL}" npx tsx maestro/fixtures/poll_for_channel.ts; then
...
~/.maestro/bin/maestro \
  ...
  maestro/flows/multi_device/device_b_verifies_channel.yml \
  --env "TEST_USER_EMAIL=${TEST_USER_EMAIL}" \
  --env "TEST_USER_PASSWORD=${TEST_USER_PASSWORD}" \
  --env "SITE_1_URL=${SITE_1_URL}" \
  --env "SYNC_TOKEN=${SYNC_TOKEN}" \
