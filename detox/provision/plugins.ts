...
async function waitForDemoPluginRunning(client: MattermostClient, token: string): Promise<void> {
    ...
    /* eslint-disable no-await-in-loop -- poll plugin status until it reports running */
    for (;;) {
        const status: PluginStatus | null = await getPluginStatus(client, token, DEMO_PLUGIN_ID);
        ...
        if (Date.now() >= deadline) {
            const detail = status ?
                `state=${status.state}${status.error ? `, error=${status.error}` : ''}` :
                'status unavailable';
...
async function getCommandVerificationTeamId(client: MattermostClient, token: string): Promise<string> {
    ...
    const createdTeam = createRes.data as Team | null;
    if (createRes.status >= 400 || !createdTeam?.id) {
        const error = createRes.data as ApiErrorBody | null;
...
async function waitForDialogCommand(client: MattermostClient, token: string, teamId: string): Promise<void> {
    ...
    const hasDialog = (commands: SlashCommand[]) => commands.some((command) => command.trigger === 'dialog');

    /* eslint-disable no-await-in-loop -- poll both command APIs until /dialog registers */
    for (;;) {
        // Collect a diagnostic per API so a failure on one is not lost when the other succeeds.
        ...
        if (commandsRes.status < 400 && Array.isArray(commandsRes.data)) {
            if (hasDialog(commandsRes.data)) {
                break;
            }
            diagnostics.push(`${commandsRes.data.length} commands`);
        } else {
            diagnostics.push(`commands HTTP ${commandsRes.status}: ${(commandsRes.data as ApiErrorBody)?.message || JSON.stringify(commandsRes.data)}`);
        }
        ...
        if (autocompleteRes.status < 400 && Array.isArray(autocompleteRes.data)) {
            if (hasDialog(autocompleteRes.data)) {
                break;
            }
            diagnostics.push(`${autocompleteRes.data.length} autocomplete commands`);
        } else {
            diagnostics.push(`autocomplete HTTP ${autocompleteRes.status}: ${(autocompleteRes.data as ApiErrorBody)?.message || JSON.stringify(autocompleteRes.data)}`);
        }

        if (Date.now() >= deadline) {
            failDemoPluginReadiness(`/dialog was not registered within ${DEMO_PLUGIN_READY_TIMEOUT_MS}ms after ${DEMO_PLUGIN_ID} started (${diagnostics.join('; ')}).`);
        }
...
export async function installRequiredPlugin(
    ...
    if (isInactive) {
        ...
        // server) must not crash the whole provision run.
        const enableRes = await client.request<ApiErrorBody>('POST', `/api/v4/plugins/${encodeURIComponent(pluginId)}/enable`, {}, token).catch((err: unknown) => ({
    ...
    // Same transient-enable-timeout guard as the inactive branch above.
