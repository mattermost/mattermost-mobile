...
describe('Threads - Reply to Thread', () => {
    ...
    it('MM-T4809_1 - should be able to reply to a thread via thread options', async () => {
        ...
        // * Verify new reply is posted (match by message — last-post can race with siblings)
        const {post: newReplyPost, error: newReplyError} = await Post.apiFindPostInChannelByMessage(
            siteOneUrl,
            testChannel.id,
            newReplyMessage,
        );
        if (!newReplyPost) {
            throw new Error(`new reply not found via API: ${JSON.stringify(newReplyError)}`);
        }
        const {postListPostItem: newReplyPostItem} = ThreadScreen.getPostListPostItem(newReplyPost.id, newReplyMessage);
        await waitFor(newReplyPostItem).toBeVisible().withTimeout(timeouts.HALF_MIN);
