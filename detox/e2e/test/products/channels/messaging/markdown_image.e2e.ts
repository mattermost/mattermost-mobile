 ...
 * MM-T4896_1 and _2 down on both platforms on b89ed6b. Sphinx rewrites `_images/` paths whenever
 * the docs rebuild, so that host is not a safe place to pin an asset. This one is a stable
 * mattermost.com upload already exercised by file_preview_gallery.e2e.ts, and at 701x701 it stays
 * under the 4096 ANDROID_MAX_WIDTH/HEIGHT cap, which is a second early return that would likewise
 * drop the testID.
 */
