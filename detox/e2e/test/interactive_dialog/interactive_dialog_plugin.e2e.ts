...
describe('Interactive Dialog - Basic Dialog (Plugin)', () => {
    ...
    it('MM-T4600 should handle date and datetime fields dialog (Plugin)', async () => {
        await ensureDialogClosed();
        await ChannelScreen.postMessage('/dialog date');
        await ensureDialogOpen();
        await InteractiveDialogScreen.fillTextElement('eventtitle', 'Team Sync Meeting');
        const dateField = element(by.id('AppFormElement.somedate.select.button'));
        await expect(dateField).toExist();
        await dateField.tap();
        await wait(500);
        ...
        await datePicker.setDatePickerDate(futureDate.toISOString(), 'ISO8601');
        await wait(500);
        await dateField.tap();
        await wait(500);
        const datetimeField = element(by.id('AppFormElement.somedatetime.select.button'));
        await expect(datetimeField).toExist();
        await datetimeField.tap();
        await wait(500);
        await expect(datePicker).toExist();
        const futureDateTime = new Date();
        futureDateTime.setDate(futureDateTime.getDate() + 14);
        futureDateTime.setHours(15, 30);
        await datePicker.setDatePickerDate(futureDateTime.toISOString(), 'ISO8601');
        await wait(500);
        await datetimeField.tap();
        await wait(500);
        await InteractiveDialogScreen.submit();
