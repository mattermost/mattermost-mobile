...
export const apiCreatePlaybook = async (baseUrl: string, playbook: Playbook): Promise<any> => {
    try {
        const response = await client.post(
            `${baseUrl}${playbooksEndpoint}`,
            playbook,
        );

        if (response.status === StatusCreated && response.headers.location) {
            // Fetch the created playbook using the location header
            const getResponse = await client.get(`${baseUrl}${response.headers.location}`);
...
export const apiCreateTestPlaybook = async (
    ...
    return apiCreatePlaybook(baseUrl, {
        team_id: teamId,
        title,
