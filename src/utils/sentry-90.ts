...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_90 = 'uncaught-app-error-90';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_90 = 'uncaught-non-error-90';

let Sentry90: any;
export function initializeSentry90() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry90) {
        Sentry90 = require('@sentry/react-native');
    }
    ...
    Sentry90.init({
        ...
        integrations: [
            Sentry90.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException90(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry90.captureException(error);
}

export function captureJSException90(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb90(error, isFatal);
    } else {
        captureException90(error);
    }
}

function captureClientErrorAsBreadcrumb90(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_90 : BREADCRUMB_UNCAUGHT_NON_ERROR_90,
        data: {
    ...
    try {
        Sentry90.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext90 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry90) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext90(database);
        Sentry90.setContext('User-Information', userContext);

        const buildContext = await getBuildTags90(database);
        Sentry90.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext90(database);
        Sentry90.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext90 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext90 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags90 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
