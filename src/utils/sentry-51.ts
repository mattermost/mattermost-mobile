...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_51 = 'uncaught-app-error-51';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_51 = 'uncaught-non-error-51';

let Sentry51: any;
export function initializeSentry51() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry51) {
        Sentry51 = require('@sentry/react-native');
    }
    ...
    Sentry51.init({
        ...
        integrations: [
            Sentry51.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException51(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry51.captureException(error);
}

export function captureJSException51(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb51(error, isFatal);
    } else {
        captureException51(error);
    }
}

function captureClientErrorAsBreadcrumb51(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_51 : BREADCRUMB_UNCAUGHT_NON_ERROR_51,
        data: {
    ...
    try {
        Sentry51.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext51 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry51) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext51(database);
        Sentry51.setContext('User-Information', userContext);

        const buildContext = await getBuildTags51(database);
        Sentry51.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext51(database);
        Sentry51.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext51 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext51 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags51 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
