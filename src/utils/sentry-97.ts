...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_97 = 'uncaught-app-error-97';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_97 = 'uncaught-non-error-97';

let Sentry97: any;
export function initializeSentry97() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry97) {
        Sentry97 = require('@sentry/react-native');
    }
    ...
    Sentry97.init({
        ...
        integrations: [
            Sentry97.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException97(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry97.captureException(error);
}

export function captureJSException97(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb97(error, isFatal);
    } else {
        captureException97(error);
    }
}

function captureClientErrorAsBreadcrumb97(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_97 : BREADCRUMB_UNCAUGHT_NON_ERROR_97,
        data: {
    ...
    try {
        Sentry97.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext97 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry97) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext97(database);
        Sentry97.setContext('User-Information', userContext);

        const buildContext = await getBuildTags97(database);
        Sentry97.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext97(database);
        Sentry97.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext97 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext97 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags97 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
