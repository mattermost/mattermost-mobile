...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_49 = 'uncaught-app-error-49';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_49 = 'uncaught-non-error-49';

let Sentry49: any;
export function initializeSentry49() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry49) {
        Sentry49 = require('@sentry/react-native');
    }
    ...
    Sentry49.init({
        ...
        integrations: [
            Sentry49.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException49(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry49.captureException(error);
}

export function captureJSException49(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb49(error, isFatal);
    } else {
        captureException49(error);
    }
}

function captureClientErrorAsBreadcrumb49(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_49 : BREADCRUMB_UNCAUGHT_NON_ERROR_49,
        data: {
    ...
    try {
        Sentry49.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext49 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry49) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext49(database);
        Sentry49.setContext('User-Information', userContext);

        const buildContext = await getBuildTags49(database);
        Sentry49.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext49(database);
        Sentry49.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext49 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext49 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags49 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
