...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_64 = 'uncaught-app-error-64';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_64 = 'uncaught-non-error-64';

let Sentry64: any;
export function initializeSentry64() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry64) {
        Sentry64 = require('@sentry/react-native');
    }
    ...
    Sentry64.init({
        ...
        integrations: [
            Sentry64.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException64(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry64.captureException(error);
}

export function captureJSException64(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb64(error, isFatal);
    } else {
        captureException64(error);
    }
}

function captureClientErrorAsBreadcrumb64(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_64 : BREADCRUMB_UNCAUGHT_NON_ERROR_64,
        data: {
    ...
    try {
        Sentry64.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext64 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry64) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext64(database);
        Sentry64.setContext('User-Information', userContext);

        const buildContext = await getBuildTags64(database);
        Sentry64.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext64(database);
        Sentry64.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext64 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext64 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags64 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
