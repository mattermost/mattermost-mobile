...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_69 = 'uncaught-app-error-69';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_69 = 'uncaught-non-error-69';

let Sentry69: any;
export function initializeSentry69() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry69) {
        Sentry69 = require('@sentry/react-native');
    }
    ...
    Sentry69.init({
        ...
        integrations: [
            Sentry69.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException69(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry69.captureException(error);
}

export function captureJSException69(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb69(error, isFatal);
    } else {
        captureException69(error);
    }
}

function captureClientErrorAsBreadcrumb69(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_69 : BREADCRUMB_UNCAUGHT_NON_ERROR_69,
        data: {
    ...
    try {
        Sentry69.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext69 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry69) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext69(database);
        Sentry69.setContext('User-Information', userContext);

        const buildContext = await getBuildTags69(database);
        Sentry69.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext69(database);
        Sentry69.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext69 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext69 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags69 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
