...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_82 = 'uncaught-app-error-82';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_82 = 'uncaught-non-error-82';

let Sentry82: any;
export function initializeSentry82() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry82) {
        Sentry82 = require('@sentry/react-native');
    }
    ...
    Sentry82.init({
        ...
        integrations: [
            Sentry82.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException82(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry82.captureException(error);
}

export function captureJSException82(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb82(error, isFatal);
    } else {
        captureException82(error);
    }
}

function captureClientErrorAsBreadcrumb82(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_82 : BREADCRUMB_UNCAUGHT_NON_ERROR_82,
        data: {
    ...
    try {
        Sentry82.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext82 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry82) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext82(database);
        Sentry82.setContext('User-Information', userContext);

        const buildContext = await getBuildTags82(database);
        Sentry82.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext82(database);
        Sentry82.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext82 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext82 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags82 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
