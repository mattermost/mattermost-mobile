...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_53 = 'uncaught-app-error-53';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_53 = 'uncaught-non-error-53';

let Sentry53: any;
export function initializeSentry53() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry53) {
        Sentry53 = require('@sentry/react-native');
    }
    ...
    Sentry53.init({
        ...
        integrations: [
            Sentry53.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException53(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry53.captureException(error);
}

export function captureJSException53(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb53(error, isFatal);
    } else {
        captureException53(error);
    }
}

function captureClientErrorAsBreadcrumb53(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_53 : BREADCRUMB_UNCAUGHT_NON_ERROR_53,
        data: {
    ...
    try {
        Sentry53.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext53 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry53) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext53(database);
        Sentry53.setContext('User-Information', userContext);

        const buildContext = await getBuildTags53(database);
        Sentry53.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext53(database);
        Sentry53.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext53 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext53 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags53 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
