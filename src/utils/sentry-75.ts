...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_75 = 'uncaught-app-error-75';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_75 = 'uncaught-non-error-75';

let Sentry75: any;
export function initializeSentry75() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry75) {
        Sentry75 = require('@sentry/react-native');
    }
    ...
    Sentry75.init({
        ...
        integrations: [
            Sentry75.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException75(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry75.captureException(error);
}

export function captureJSException75(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb75(error, isFatal);
    } else {
        captureException75(error);
    }
}

function captureClientErrorAsBreadcrumb75(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_75 : BREADCRUMB_UNCAUGHT_NON_ERROR_75,
        data: {
    ...
    try {
        Sentry75.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext75 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry75) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext75(database);
        Sentry75.setContext('User-Information', userContext);

        const buildContext = await getBuildTags75(database);
        Sentry75.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext75(database);
        Sentry75.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext75 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext75 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags75 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
