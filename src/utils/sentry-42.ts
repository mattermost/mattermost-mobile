...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_42 = 'uncaught-app-error-42';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_42 = 'uncaught-non-error-42';

let Sentry42: any;
export function initializeSentry42() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry42) {
        Sentry42 = require('@sentry/react-native');
    }
    ...
    Sentry42.init({
        ...
        integrations: [
            Sentry42.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException42(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry42.captureException(error);
}

export function captureJSException42(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb42(error, isFatal);
    } else {
        captureException42(error);
    }
}

function captureClientErrorAsBreadcrumb42(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_42 : BREADCRUMB_UNCAUGHT_NON_ERROR_42,
        data: {
    ...
    try {
        Sentry42.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext42 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry42) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext42(database);
        Sentry42.setContext('User-Information', userContext);

        const buildContext = await getBuildTags42(database);
        Sentry42.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext42(database);
        Sentry42.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext42 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext42 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags42 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
