...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_15 = 'uncaught-app-error-15';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_15 = 'uncaught-non-error-15';

let Sentry15: any;
export function initializeSentry15() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry15) {
        Sentry15 = require('@sentry/react-native');
    }
    ...
    Sentry15.init({
        ...
        integrations: [
            Sentry15.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException15(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry15.captureException(error);
}

export function captureJSException15(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb15(error, isFatal);
    } else {
        captureException15(error);
    }
}

function captureClientErrorAsBreadcrumb15(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_15 : BREADCRUMB_UNCAUGHT_NON_ERROR_15,
        data: {
    ...
    try {
        Sentry15.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext15 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry15) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext15(database);
        Sentry15.setContext('User-Information', userContext);

        const buildContext = await getBuildTags15(database);
        Sentry15.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext15(database);
        Sentry15.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext15 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext15 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags15 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
