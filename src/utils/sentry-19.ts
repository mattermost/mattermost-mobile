...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_19 = 'uncaught-app-error-19';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_19 = 'uncaught-non-error-19';

let Sentry19: any;
export function initializeSentry19() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry19) {
        Sentry19 = require('@sentry/react-native');
    }
    ...
    Sentry19.init({
        ...
        integrations: [
            Sentry19.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException19(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry19.captureException(error);
}

export function captureJSException19(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb19(error, isFatal);
    } else {
        captureException19(error);
    }
}

function captureClientErrorAsBreadcrumb19(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_19 : BREADCRUMB_UNCAUGHT_NON_ERROR_19,
        data: {
    ...
    try {
        Sentry19.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext19 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry19) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext19(database);
        Sentry19.setContext('User-Information', userContext);

        const buildContext = await getBuildTags19(database);
        Sentry19.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext19(database);
        Sentry19.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext19 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext19 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags19 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
