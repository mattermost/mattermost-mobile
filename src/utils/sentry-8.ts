...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_8 = 'uncaught-app-error-8';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_8 = 'uncaught-non-error-8';

let Sentry8: any;
export function initializeSentry8() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry8) {
        Sentry8 = require('@sentry/react-native');
    }
    ...
    Sentry8.init({
        ...
        integrations: [
            Sentry8.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException8(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry8.captureException(error);
}

export function captureJSException8(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb8(error, isFatal);
    } else {
        captureException8(error);
    }
}

function captureClientErrorAsBreadcrumb8(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_8 : BREADCRUMB_UNCAUGHT_NON_ERROR_8,
        data: {
    ...
    try {
        Sentry8.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext8 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry8) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext8(database);
        Sentry8.setContext('User-Information', userContext);

        const buildContext = await getBuildTags8(database);
        Sentry8.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext8(database);
        Sentry8.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext8 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext8 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags8 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
