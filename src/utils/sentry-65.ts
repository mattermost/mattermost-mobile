...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_65 = 'uncaught-app-error-65';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_65 = 'uncaught-non-error-65';

let Sentry65: any;
export function initializeSentry65() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry65) {
        Sentry65 = require('@sentry/react-native');
    }
    ...
    Sentry65.init({
        ...
        integrations: [
            Sentry65.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException65(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry65.captureException(error);
}

export function captureJSException65(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb65(error, isFatal);
    } else {
        captureException65(error);
    }
}

function captureClientErrorAsBreadcrumb65(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_65 : BREADCRUMB_UNCAUGHT_NON_ERROR_65,
        data: {
    ...
    try {
        Sentry65.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext65 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry65) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext65(database);
        Sentry65.setContext('User-Information', userContext);

        const buildContext = await getBuildTags65(database);
        Sentry65.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext65(database);
        Sentry65.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext65 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext65 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags65 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
