...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_78 = 'uncaught-app-error-78';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_78 = 'uncaught-non-error-78';

let Sentry78: any;
export function initializeSentry78() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry78) {
        Sentry78 = require('@sentry/react-native');
    }
    ...
    Sentry78.init({
        ...
        integrations: [
            Sentry78.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException78(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry78.captureException(error);
}

export function captureJSException78(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb78(error, isFatal);
    } else {
        captureException78(error);
    }
}

function captureClientErrorAsBreadcrumb78(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_78 : BREADCRUMB_UNCAUGHT_NON_ERROR_78,
        data: {
    ...
    try {
        Sentry78.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext78 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry78) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext78(database);
        Sentry78.setContext('User-Information', userContext);

        const buildContext = await getBuildTags78(database);
        Sentry78.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext78(database);
        Sentry78.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext78 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext78 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags78 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
