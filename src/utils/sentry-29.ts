...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_29 = 'uncaught-app-error-29';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_29 = 'uncaught-non-error-29';

let Sentry29: any;
export function initializeSentry29() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry29) {
        Sentry29 = require('@sentry/react-native');
    }
    ...
    Sentry29.init({
        ...
        integrations: [
            Sentry29.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException29(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry29.captureException(error);
}

export function captureJSException29(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb29(error, isFatal);
    } else {
        captureException29(error);
    }
}

function captureClientErrorAsBreadcrumb29(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_29 : BREADCRUMB_UNCAUGHT_NON_ERROR_29,
        data: {
    ...
    try {
        Sentry29.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext29 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry29) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext29(database);
        Sentry29.setContext('User-Information', userContext);

        const buildContext = await getBuildTags29(database);
        Sentry29.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext29(database);
        Sentry29.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext29 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext29 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags29 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
