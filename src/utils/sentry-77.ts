...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_77 = 'uncaught-app-error-77';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_77 = 'uncaught-non-error-77';

let Sentry77: any;
export function initializeSentry77() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry77) {
        Sentry77 = require('@sentry/react-native');
    }
    ...
    Sentry77.init({
        ...
        integrations: [
            Sentry77.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException77(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry77.captureException(error);
}

export function captureJSException77(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb77(error, isFatal);
    } else {
        captureException77(error);
    }
}

function captureClientErrorAsBreadcrumb77(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_77 : BREADCRUMB_UNCAUGHT_NON_ERROR_77,
        data: {
    ...
    try {
        Sentry77.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext77 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry77) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext77(database);
        Sentry77.setContext('User-Information', userContext);

        const buildContext = await getBuildTags77(database);
        Sentry77.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext77(database);
        Sentry77.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext77 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext77 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags77 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
