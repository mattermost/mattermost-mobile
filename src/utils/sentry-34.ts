...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_34 = 'uncaught-app-error-34';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_34 = 'uncaught-non-error-34';

let Sentry34: any;
export function initializeSentry34() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry34) {
        Sentry34 = require('@sentry/react-native');
    }
    ...
    Sentry34.init({
        ...
        integrations: [
            Sentry34.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException34(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry34.captureException(error);
}

export function captureJSException34(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb34(error, isFatal);
    } else {
        captureException34(error);
    }
}

function captureClientErrorAsBreadcrumb34(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_34 : BREADCRUMB_UNCAUGHT_NON_ERROR_34,
        data: {
    ...
    try {
        Sentry34.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext34 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry34) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext34(database);
        Sentry34.setContext('User-Information', userContext);

        const buildContext = await getBuildTags34(database);
        Sentry34.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext34(database);
        Sentry34.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext34 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext34 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags34 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
