...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_12 = 'uncaught-app-error-12';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_12 = 'uncaught-non-error-12';

let Sentry12: any;
export function initializeSentry12() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry12) {
        Sentry12 = require('@sentry/react-native');
    }
    ...
    Sentry12.init({
        ...
        integrations: [
            Sentry12.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException12(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry12.captureException(error);
}

export function captureJSException12(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb12(error, isFatal);
    } else {
        captureException12(error);
    }
}

function captureClientErrorAsBreadcrumb12(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_12 : BREADCRUMB_UNCAUGHT_NON_ERROR_12,
        data: {
    ...
    try {
        Sentry12.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext12 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry12) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext12(database);
        Sentry12.setContext('User-Information', userContext);

        const buildContext = await getBuildTags12(database);
        Sentry12.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext12(database);
        Sentry12.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext12 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext12 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags12 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
