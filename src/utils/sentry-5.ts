...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_5 = 'uncaught-app-error-5';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_5 = 'uncaught-non-error-5';

let Sentry5: any;
export function initializeSentry5() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry5) {
        Sentry5 = require('@sentry/react-native');
    }
    ...
    Sentry5.init({
        ...
        integrations: [
            Sentry5.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException5(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
...
export function captureJSException5(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb5(error, isFatal);
    } else {
        captureException5(error);
    }
}

function captureClientErrorAsBreadcrumb5(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_5 : BREADCRUMB_UNCAUGHT_NON_ERROR_5,
        data: {
    ...
    try {
        Sentry5.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext5 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry5) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext5(database);
        Sentry5.setContext('User-Information', userContext);

        const buildContext = await getBuildTags5(database);
        Sentry5.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext5(database);
        Sentry5.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext5 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext5 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags5 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
