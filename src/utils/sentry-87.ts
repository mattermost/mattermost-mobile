...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_87 = 'uncaught-app-error-87';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_87 = 'uncaught-non-error-87';

let Sentry87: any;
export function initializeSentry87() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry87) {
        Sentry87 = require('@sentry/react-native');
    }
    ...
    Sentry87.init({
        ...
        integrations: [
            Sentry87.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException87(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry87.captureException(error);
}

export function captureJSException87(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb87(error, isFatal);
    } else {
        captureException87(error);
    }
}

function captureClientErrorAsBreadcrumb87(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_87 : BREADCRUMB_UNCAUGHT_NON_ERROR_87,
        data: {
    ...
    try {
        Sentry87.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext87 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry87) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext87(database);
        Sentry87.setContext('User-Information', userContext);

        const buildContext = await getBuildTags87(database);
        Sentry87.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext87(database);
        Sentry87.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext87 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext87 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags87 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
