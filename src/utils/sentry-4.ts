...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_4 = 'uncaught-app-error-4';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_4 = 'uncaught-non-error-4';

let Sentry4: any;
export function initializeSentry4() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry4) {
        Sentry4 = require('@sentry/react-native');
    }
    ...
    Sentry4.init({
        ...
        integrations: [
            Sentry4.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException4(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
...
export function captureJSException4(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb4(error, isFatal);
    } else {
        captureException4(error);
    }
}

function captureClientErrorAsBreadcrumb4(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_4 : BREADCRUMB_UNCAUGHT_NON_ERROR_4,
        data: {
    ...
    try {
        Sentry4.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext4 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry4) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext4(database);
        Sentry4.setContext('User-Information', userContext);

        const buildContext = await getBuildTags4(database);
        Sentry4.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext4(database);
        Sentry4.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext4 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext4 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags4 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
