...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_39 = 'uncaught-app-error-39';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_39 = 'uncaught-non-error-39';

let Sentry39: any;
export function initializeSentry39() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry39) {
        Sentry39 = require('@sentry/react-native');
    }
    ...
    Sentry39.init({
        ...
        integrations: [
            Sentry39.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException39(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry39.captureException(error);
}

export function captureJSException39(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb39(error, isFatal);
    } else {
        captureException39(error);
    }
}

function captureClientErrorAsBreadcrumb39(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_39 : BREADCRUMB_UNCAUGHT_NON_ERROR_39,
        data: {
    ...
    try {
        Sentry39.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext39 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry39) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext39(database);
        Sentry39.setContext('User-Information', userContext);

        const buildContext = await getBuildTags39(database);
        Sentry39.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext39(database);
        Sentry39.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext39 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext39 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags39 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
