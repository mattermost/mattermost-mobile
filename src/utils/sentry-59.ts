...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_59 = 'uncaught-app-error-59';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_59 = 'uncaught-non-error-59';

let Sentry59: any;
export function initializeSentry59() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry59) {
        Sentry59 = require('@sentry/react-native');
    }
    ...
    Sentry59.init({
        ...
        integrations: [
            Sentry59.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException59(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry59.captureException(error);
}

export function captureJSException59(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb59(error, isFatal);
    } else {
        captureException59(error);
    }
}

function captureClientErrorAsBreadcrumb59(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_59 : BREADCRUMB_UNCAUGHT_NON_ERROR_59,
        data: {
    ...
    try {
        Sentry59.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext59 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry59) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext59(database);
        Sentry59.setContext('User-Information', userContext);

        const buildContext = await getBuildTags59(database);
        Sentry59.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext59(database);
        Sentry59.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext59 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext59 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags59 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
