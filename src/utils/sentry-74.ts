...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_74 = 'uncaught-app-error-74';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_74 = 'uncaught-non-error-74';

let Sentry74: any;
export function initializeSentry74() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry74) {
        Sentry74 = require('@sentry/react-native');
    }
    ...
    Sentry74.init({
        ...
        integrations: [
            Sentry74.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException74(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry74.captureException(error);
}

export function captureJSException74(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb74(error, isFatal);
    } else {
        captureException74(error);
    }
}

function captureClientErrorAsBreadcrumb74(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_74 : BREADCRUMB_UNCAUGHT_NON_ERROR_74,
        data: {
    ...
    try {
        Sentry74.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext74 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry74) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext74(database);
        Sentry74.setContext('User-Information', userContext);

        const buildContext = await getBuildTags74(database);
        Sentry74.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext74(database);
        Sentry74.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext74 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext74 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags74 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
