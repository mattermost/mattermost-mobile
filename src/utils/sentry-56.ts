...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_56 = 'uncaught-app-error-56';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_56 = 'uncaught-non-error-56';

let Sentry56: any;
export function initializeSentry56() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry56) {
        Sentry56 = require('@sentry/react-native');
    }
    ...
    Sentry56.init({
        ...
        integrations: [
            Sentry56.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException56(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry56.captureException(error);
}

export function captureJSException56(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb56(error, isFatal);
    } else {
        captureException56(error);
    }
}

function captureClientErrorAsBreadcrumb56(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_56 : BREADCRUMB_UNCAUGHT_NON_ERROR_56,
        data: {
    ...
    try {
        Sentry56.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext56 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry56) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext56(database);
        Sentry56.setContext('User-Information', userContext);

        const buildContext = await getBuildTags56(database);
        Sentry56.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext56(database);
        Sentry56.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext56 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext56 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags56 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
