...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_21 = 'uncaught-app-error-21';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_21 = 'uncaught-non-error-21';

let Sentry21: any;
export function initializeSentry21() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry21) {
        Sentry21 = require('@sentry/react-native');
    }
    ...
    Sentry21.init({
        ...
        integrations: [
            Sentry21.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException21(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry21.captureException(error);
}

export function captureJSException21(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb21(error, isFatal);
    } else {
        captureException21(error);
    }
}

function captureClientErrorAsBreadcrumb21(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_21 : BREADCRUMB_UNCAUGHT_NON_ERROR_21,
        data: {
    ...
    try {
        Sentry21.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext21 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry21) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext21(database);
        Sentry21.setContext('User-Information', userContext);

        const buildContext = await getBuildTags21(database);
        Sentry21.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext21(database);
        Sentry21.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext21 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext21 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags21 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
