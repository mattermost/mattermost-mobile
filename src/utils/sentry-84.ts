...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_84 = 'uncaught-app-error-84';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_84 = 'uncaught-non-error-84';

let Sentry84: any;
export function initializeSentry84() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry84) {
        Sentry84 = require('@sentry/react-native');
    }
    ...
    Sentry84.init({
        ...
        integrations: [
            Sentry84.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException84(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry84.captureException(error);
}

export function captureJSException84(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb84(error, isFatal);
    } else {
        captureException84(error);
    }
}

function captureClientErrorAsBreadcrumb84(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_84 : BREADCRUMB_UNCAUGHT_NON_ERROR_84,
        data: {
    ...
    try {
        Sentry84.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext84 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry84) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext84(database);
        Sentry84.setContext('User-Information', userContext);

        const buildContext = await getBuildTags84(database);
        Sentry84.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext84(database);
        Sentry84.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext84 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext84 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags84 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
