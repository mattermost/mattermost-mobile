...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_16 = 'uncaught-app-error-16';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_16 = 'uncaught-non-error-16';

let Sentry16: any;
export function initializeSentry16() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry16) {
        Sentry16 = require('@sentry/react-native');
    }
    ...
    Sentry16.init({
        ...
        integrations: [
            Sentry16.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException16(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry16.captureException(error);
}

export function captureJSException16(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb16(error, isFatal);
    } else {
        captureException16(error);
    }
}

function captureClientErrorAsBreadcrumb16(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_16 : BREADCRUMB_UNCAUGHT_NON_ERROR_16,
        data: {
    ...
    try {
        Sentry16.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext16 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry16) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext16(database);
        Sentry16.setContext('User-Information', userContext);

        const buildContext = await getBuildTags16(database);
        Sentry16.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext16(database);
        Sentry16.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext16 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext16 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags16 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
