...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_28 = 'uncaught-app-error-28';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_28 = 'uncaught-non-error-28';

let Sentry28: any;
export function initializeSentry28() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry28) {
        Sentry28 = require('@sentry/react-native');
    }
    ...
    Sentry28.init({
        ...
        integrations: [
            Sentry28.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException28(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry28.captureException(error);
}

export function captureJSException28(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb28(error, isFatal);
    } else {
        captureException28(error);
    }
}

function captureClientErrorAsBreadcrumb28(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_28 : BREADCRUMB_UNCAUGHT_NON_ERROR_28,
        data: {
    ...
    try {
        Sentry28.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext28 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry28) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext28(database);
        Sentry28.setContext('User-Information', userContext);

        const buildContext = await getBuildTags28(database);
        Sentry28.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext28(database);
        Sentry28.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext28 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext28 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags28 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
