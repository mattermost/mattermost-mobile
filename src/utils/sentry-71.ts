...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_71 = 'uncaught-app-error-71';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_71 = 'uncaught-non-error-71';

let Sentry71: any;
export function initializeSentry71() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry71) {
        Sentry71 = require('@sentry/react-native');
    }
    ...
    Sentry71.init({
        ...
        integrations: [
            Sentry71.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException71(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry71.captureException(error);
}

export function captureJSException71(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb71(error, isFatal);
    } else {
        captureException71(error);
    }
}

function captureClientErrorAsBreadcrumb71(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_71 : BREADCRUMB_UNCAUGHT_NON_ERROR_71,
        data: {
    ...
    try {
        Sentry71.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext71 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry71) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext71(database);
        Sentry71.setContext('User-Information', userContext);

        const buildContext = await getBuildTags71(database);
        Sentry71.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext71(database);
        Sentry71.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext71 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext71 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags71 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
