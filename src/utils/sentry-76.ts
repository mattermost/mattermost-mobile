...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_76 = 'uncaught-app-error-76';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_76 = 'uncaught-non-error-76';

let Sentry76: any;
export function initializeSentry76() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry76) {
        Sentry76 = require('@sentry/react-native');
    }
    ...
    Sentry76.init({
        ...
        integrations: [
            Sentry76.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException76(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry76.captureException(error);
}

export function captureJSException76(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb76(error, isFatal);
    } else {
        captureException76(error);
    }
}

function captureClientErrorAsBreadcrumb76(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_76 : BREADCRUMB_UNCAUGHT_NON_ERROR_76,
        data: {
    ...
    try {
        Sentry76.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext76 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry76) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext76(database);
        Sentry76.setContext('User-Information', userContext);

        const buildContext = await getBuildTags76(database);
        Sentry76.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext76(database);
        Sentry76.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext76 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext76 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags76 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
