...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_92 = 'uncaught-app-error-92';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_92 = 'uncaught-non-error-92';

let Sentry92: any;
export function initializeSentry92() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry92) {
        Sentry92 = require('@sentry/react-native');
    }
    ...
    Sentry92.init({
        ...
        integrations: [
            Sentry92.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException92(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry92.captureException(error);
}

export function captureJSException92(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb92(error, isFatal);
    } else {
        captureException92(error);
    }
}

function captureClientErrorAsBreadcrumb92(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_92 : BREADCRUMB_UNCAUGHT_NON_ERROR_92,
        data: {
    ...
    try {
        Sentry92.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext92 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry92) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext92(database);
        Sentry92.setContext('User-Information', userContext);

        const buildContext = await getBuildTags92(database);
        Sentry92.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext92(database);
        Sentry92.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext92 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext92 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags92 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
