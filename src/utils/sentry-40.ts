...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_40 = 'uncaught-app-error-40';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_40 = 'uncaught-non-error-40';

let Sentry40: any;
export function initializeSentry40() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry40) {
        Sentry40 = require('@sentry/react-native');
    }
    ...
    Sentry40.init({
        ...
        integrations: [
            Sentry40.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException40(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry40.captureException(error);
}

export function captureJSException40(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb40(error, isFatal);
    } else {
        captureException40(error);
    }
}

function captureClientErrorAsBreadcrumb40(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_40 : BREADCRUMB_UNCAUGHT_NON_ERROR_40,
        data: {
    ...
    try {
        Sentry40.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext40 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry40) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext40(database);
        Sentry40.setContext('User-Information', userContext);

        const buildContext = await getBuildTags40(database);
        Sentry40.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext40(database);
        Sentry40.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext40 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext40 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags40 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
