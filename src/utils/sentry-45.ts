...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_45 = 'uncaught-app-error-45';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_45 = 'uncaught-non-error-45';

let Sentry45: any;
export function initializeSentry45() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry45) {
        Sentry45 = require('@sentry/react-native');
    }
    ...
    Sentry45.init({
        ...
        integrations: [
            Sentry45.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException45(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry45.captureException(error);
}

export function captureJSException45(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb45(error, isFatal);
    } else {
        captureException45(error);
    }
}

function captureClientErrorAsBreadcrumb45(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_45 : BREADCRUMB_UNCAUGHT_NON_ERROR_45,
        data: {
    ...
    try {
        Sentry45.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext45 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry45) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext45(database);
        Sentry45.setContext('User-Information', userContext);

        const buildContext = await getBuildTags45(database);
        Sentry45.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext45(database);
        Sentry45.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext45 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext45 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags45 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
