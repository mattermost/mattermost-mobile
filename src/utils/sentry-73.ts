...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_73 = 'uncaught-app-error-73';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_73 = 'uncaught-non-error-73';

let Sentry73: any;
export function initializeSentry73() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry73) {
        Sentry73 = require('@sentry/react-native');
    }
    ...
    Sentry73.init({
        ...
        integrations: [
            Sentry73.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException73(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry73.captureException(error);
}

export function captureJSException73(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb73(error, isFatal);
    } else {
        captureException73(error);
    }
}

function captureClientErrorAsBreadcrumb73(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_73 : BREADCRUMB_UNCAUGHT_NON_ERROR_73,
        data: {
    ...
    try {
        Sentry73.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext73 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry73) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext73(database);
        Sentry73.setContext('User-Information', userContext);

        const buildContext = await getBuildTags73(database);
        Sentry73.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext73(database);
        Sentry73.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext73 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext73 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags73 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
