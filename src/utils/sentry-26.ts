...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_26 = 'uncaught-app-error-26';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_26 = 'uncaught-non-error-26';

let Sentry26: any;
export function initializeSentry26() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry26) {
        Sentry26 = require('@sentry/react-native');
    }
    ...
    Sentry26.init({
        ...
        integrations: [
            Sentry26.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException26(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry26.captureException(error);
}

export function captureJSException26(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb26(error, isFatal);
    } else {
        captureException26(error);
    }
}

function captureClientErrorAsBreadcrumb26(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_26 : BREADCRUMB_UNCAUGHT_NON_ERROR_26,
        data: {
    ...
    try {
        Sentry26.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext26 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry26) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext26(database);
        Sentry26.setContext('User-Information', userContext);

        const buildContext = await getBuildTags26(database);
        Sentry26.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext26(database);
        Sentry26.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext26 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext26 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags26 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
