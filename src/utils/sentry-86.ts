...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_86 = 'uncaught-app-error-86';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_86 = 'uncaught-non-error-86';

let Sentry86: any;
export function initializeSentry86() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry86) {
        Sentry86 = require('@sentry/react-native');
    }
    ...
    Sentry86.init({
        ...
        integrations: [
            Sentry86.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException86(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry86.captureException(error);
}

export function captureJSException86(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb86(error, isFatal);
    } else {
        captureException86(error);
    }
}

function captureClientErrorAsBreadcrumb86(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_86 : BREADCRUMB_UNCAUGHT_NON_ERROR_86,
        data: {
    ...
    try {
        Sentry86.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext86 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry86) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext86(database);
        Sentry86.setContext('User-Information', userContext);

        const buildContext = await getBuildTags86(database);
        Sentry86.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext86(database);
        Sentry86.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext86 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext86 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags86 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
