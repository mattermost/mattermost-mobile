...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_6 = 'uncaught-app-error-6';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_6 = 'uncaught-non-error-6';

let Sentry6: any;
export function initializeSentry6() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry6) {
        Sentry6 = require('@sentry/react-native');
    }
    ...
    Sentry6.init({
        ...
        integrations: [
            Sentry6.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException6(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry6.captureException(error);
}

export function captureJSException6(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb6(error, isFatal);
    } else {
        captureException6(error);
    }
}

function captureClientErrorAsBreadcrumb6(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_6 : BREADCRUMB_UNCAUGHT_NON_ERROR_6,
        data: {
    ...
    try {
        Sentry6.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext6 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry6) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext6(database);
        Sentry6.setContext('User-Information', userContext);

        const buildContext = await getBuildTags6(database);
        Sentry6.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext6(database);
        Sentry6.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext6 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext6 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags6 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
