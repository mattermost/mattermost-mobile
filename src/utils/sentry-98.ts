...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_98 = 'uncaught-app-error-98';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_98 = 'uncaught-non-error-98';

let Sentry98: any;
export function initializeSentry98() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry98) {
        Sentry98 = require('@sentry/react-native');
    }
    ...
    Sentry98.init({
        ...
        integrations: [
            Sentry98.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException98(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry98.captureException(error);
}

export function captureJSException98(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb98(error, isFatal);
    } else {
        captureException98(error);
    }
}

function captureClientErrorAsBreadcrumb98(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_98 : BREADCRUMB_UNCAUGHT_NON_ERROR_98,
        data: {
    ...
    try {
        Sentry98.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext98 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry98) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext98(database);
        Sentry98.setContext('User-Information', userContext);

        const buildContext = await getBuildTags98(database);
        Sentry98.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext98(database);
        Sentry98.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext98 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext98 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags98 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
