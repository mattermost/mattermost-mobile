...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_50 = 'uncaught-app-error-50';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_50 = 'uncaught-non-error-50';

let Sentry50: any;
export function initializeSentry50() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry50) {
        Sentry50 = require('@sentry/react-native');
    }
    ...
    Sentry50.init({
        ...
        integrations: [
            Sentry50.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException50(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry50.captureException(error);
}

export function captureJSException50(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb50(error, isFatal);
    } else {
        captureException50(error);
    }
}

function captureClientErrorAsBreadcrumb50(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_50 : BREADCRUMB_UNCAUGHT_NON_ERROR_50,
        data: {
    ...
    try {
        Sentry50.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext50 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry50) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext50(database);
        Sentry50.setContext('User-Information', userContext);

        const buildContext = await getBuildTags50(database);
        Sentry50.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext50(database);
        Sentry50.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext50 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext50 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags50 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
