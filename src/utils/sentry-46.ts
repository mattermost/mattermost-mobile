...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_46 = 'uncaught-app-error-46';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_46 = 'uncaught-non-error-46';

let Sentry46: any;
export function initializeSentry46() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry46) {
        Sentry46 = require('@sentry/react-native');
    }
    ...
    Sentry46.init({
        ...
        integrations: [
            Sentry46.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException46(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry46.captureException(error);
}

export function captureJSException46(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb46(error, isFatal);
    } else {
        captureException46(error);
    }
}

function captureClientErrorAsBreadcrumb46(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_46 : BREADCRUMB_UNCAUGHT_NON_ERROR_46,
        data: {
    ...
    try {
        Sentry46.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext46 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry46) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext46(database);
        Sentry46.setContext('User-Information', userContext);

        const buildContext = await getBuildTags46(database);
        Sentry46.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext46(database);
        Sentry46.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext46 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext46 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags46 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
