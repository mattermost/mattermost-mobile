...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_91 = 'uncaught-app-error-91';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_91 = 'uncaught-non-error-91';

let Sentry91: any;
export function initializeSentry91() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry91) {
        Sentry91 = require('@sentry/react-native');
    }
    ...
    Sentry91.init({
        ...
        integrations: [
            Sentry91.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException91(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry91.captureException(error);
}

export function captureJSException91(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb91(error, isFatal);
    } else {
        captureException91(error);
    }
}

function captureClientErrorAsBreadcrumb91(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_91 : BREADCRUMB_UNCAUGHT_NON_ERROR_91,
        data: {
    ...
    try {
        Sentry91.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext91 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry91) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext91(database);
        Sentry91.setContext('User-Information', userContext);

        const buildContext = await getBuildTags91(database);
        Sentry91.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext91(database);
        Sentry91.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext91 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext91 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags91 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
