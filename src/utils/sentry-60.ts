...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_60 = 'uncaught-app-error-60';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_60 = 'uncaught-non-error-60';

let Sentry60: any;
export function initializeSentry60() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry60) {
        Sentry60 = require('@sentry/react-native');
    }
    ...
    Sentry60.init({
        ...
        integrations: [
            Sentry60.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException60(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry60.captureException(error);
}

export function captureJSException60(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb60(error, isFatal);
    } else {
        captureException60(error);
    }
}

function captureClientErrorAsBreadcrumb60(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_60 : BREADCRUMB_UNCAUGHT_NON_ERROR_60,
        data: {
    ...
    try {
        Sentry60.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext60 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry60) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext60(database);
        Sentry60.setContext('User-Information', userContext);

        const buildContext = await getBuildTags60(database);
        Sentry60.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext60(database);
        Sentry60.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext60 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext60 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags60 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
