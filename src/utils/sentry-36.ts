...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_36 = 'uncaught-app-error-36';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_36 = 'uncaught-non-error-36';

let Sentry36: any;
export function initializeSentry36() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry36) {
        Sentry36 = require('@sentry/react-native');
    }
    ...
    Sentry36.init({
        ...
        integrations: [
            Sentry36.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException36(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry36.captureException(error);
}

export function captureJSException36(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb36(error, isFatal);
    } else {
        captureException36(error);
    }
}

function captureClientErrorAsBreadcrumb36(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_36 : BREADCRUMB_UNCAUGHT_NON_ERROR_36,
        data: {
    ...
    try {
        Sentry36.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext36 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry36) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext36(database);
        Sentry36.setContext('User-Information', userContext);

        const buildContext = await getBuildTags36(database);
        Sentry36.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext36(database);
        Sentry36.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext36 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext36 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags36 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
