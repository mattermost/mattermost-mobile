...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_48 = 'uncaught-app-error-48';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_48 = 'uncaught-non-error-48';

let Sentry48: any;
export function initializeSentry48() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry48) {
        Sentry48 = require('@sentry/react-native');
    }
    ...
    Sentry48.init({
        ...
        integrations: [
            Sentry48.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException48(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry48.captureException(error);
}

export function captureJSException48(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb48(error, isFatal);
    } else {
        captureException48(error);
    }
}

function captureClientErrorAsBreadcrumb48(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_48 : BREADCRUMB_UNCAUGHT_NON_ERROR_48,
        data: {
    ...
    try {
        Sentry48.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext48 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry48) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext48(database);
        Sentry48.setContext('User-Information', userContext);

        const buildContext = await getBuildTags48(database);
        Sentry48.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext48(database);
        Sentry48.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext48 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext48 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags48 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
