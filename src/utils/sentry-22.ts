...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_22 = 'uncaught-app-error-22';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_22 = 'uncaught-non-error-22';

let Sentry22: any;
export function initializeSentry22() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry22) {
        Sentry22 = require('@sentry/react-native');
    }
    ...
    Sentry22.init({
        ...
        integrations: [
            Sentry22.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException22(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry22.captureException(error);
}

export function captureJSException22(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb22(error, isFatal);
    } else {
        captureException22(error);
    }
}

function captureClientErrorAsBreadcrumb22(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_22 : BREADCRUMB_UNCAUGHT_NON_ERROR_22,
        data: {
    ...
    try {
        Sentry22.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext22 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry22) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext22(database);
        Sentry22.setContext('User-Information', userContext);

        const buildContext = await getBuildTags22(database);
        Sentry22.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext22(database);
        Sentry22.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext22 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext22 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags22 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
