...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_1 = 'uncaught-app-error-1';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_1 = 'uncaught-non-error-1';

let Sentry1: any;
export function initializeSentry1() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry1) {
        Sentry1 = require('@sentry/react-native');
    }
    ...
    Sentry1.init({
        ...
        integrations: [
            Sentry1.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException1(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry1.captureException(error);
}

export function captureJSException1(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb1(error, isFatal);
    } else {
        captureException1(error);
    }
}

function captureClientErrorAsBreadcrumb1(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_1 : BREADCRUMB_UNCAUGHT_NON_ERROR_1,
        data: {
    ...
    try {
        Sentry1.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext1 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry1) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext1(database);
        Sentry1.setContext('User-Information', userContext);

        const buildContext = await getBuildTags1(database);
        Sentry1.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext1(database);
        Sentry1.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext1 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext1 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags1 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
