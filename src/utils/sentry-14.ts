...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_14 = 'uncaught-app-error-14';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_14 = 'uncaught-non-error-14';

let Sentry14: any;
export function initializeSentry14() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry14) {
        Sentry14 = require('@sentry/react-native');
    }
    ...
    Sentry14.init({
        ...
        integrations: [
            Sentry14.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException14(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry14.captureException(error);
}

export function captureJSException14(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb14(error, isFatal);
    } else {
        captureException14(error);
    }
}

function captureClientErrorAsBreadcrumb14(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_14 : BREADCRUMB_UNCAUGHT_NON_ERROR_14,
        data: {
    ...
    try {
        Sentry14.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext14 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry14) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext14(database);
        Sentry14.setContext('User-Information', userContext);

        const buildContext = await getBuildTags14(database);
        Sentry14.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext14(database);
        Sentry14.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext14 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext14 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags14 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
