...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_30 = 'uncaught-app-error-30';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_30 = 'uncaught-non-error-30';

let Sentry30: any;
export function initializeSentry30() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry30) {
        Sentry30 = require('@sentry/react-native');
    }
    ...
    Sentry30.init({
        ...
        integrations: [
            Sentry30.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException30(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry30.captureException(error);
}

export function captureJSException30(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb30(error, isFatal);
    } else {
        captureException30(error);
    }
}

function captureClientErrorAsBreadcrumb30(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_30 : BREADCRUMB_UNCAUGHT_NON_ERROR_30,
        data: {
    ...
    try {
        Sentry30.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext30 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry30) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext30(database);
        Sentry30.setContext('User-Information', userContext);

        const buildContext = await getBuildTags30(database);
        Sentry30.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext30(database);
        Sentry30.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext30 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext30 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags30 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
