...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_52 = 'uncaught-app-error-52';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_52 = 'uncaught-non-error-52';

let Sentry52: any;
export function initializeSentry52() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry52) {
        Sentry52 = require('@sentry/react-native');
    }
    ...
    Sentry52.init({
        ...
        integrations: [
            Sentry52.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException52(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry52.captureException(error);
}

export function captureJSException52(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb52(error, isFatal);
    } else {
        captureException52(error);
    }
}

function captureClientErrorAsBreadcrumb52(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_52 : BREADCRUMB_UNCAUGHT_NON_ERROR_52,
        data: {
    ...
    try {
        Sentry52.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext52 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry52) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext52(database);
        Sentry52.setContext('User-Information', userContext);

        const buildContext = await getBuildTags52(database);
        Sentry52.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext52(database);
        Sentry52.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext52 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext52 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags52 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
