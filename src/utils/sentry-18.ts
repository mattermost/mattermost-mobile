...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_18 = 'uncaught-app-error-18';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_18 = 'uncaught-non-error-18';

let Sentry18: any;
export function initializeSentry18() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry18) {
        Sentry18 = require('@sentry/react-native');
    }
    ...
    Sentry18.init({
        ...
        integrations: [
            Sentry18.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException18(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry18.captureException(error);
}

export function captureJSException18(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb18(error, isFatal);
    } else {
        captureException18(error);
    }
}

function captureClientErrorAsBreadcrumb18(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_18 : BREADCRUMB_UNCAUGHT_NON_ERROR_18,
        data: {
    ...
    try {
        Sentry18.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext18 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry18) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext18(database);
        Sentry18.setContext('User-Information', userContext);

        const buildContext = await getBuildTags18(database);
        Sentry18.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext18(database);
        Sentry18.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext18 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext18 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags18 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
