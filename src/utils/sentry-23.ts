...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_23 = 'uncaught-app-error-23';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_23 = 'uncaught-non-error-23';

let Sentry23: any;
export function initializeSentry23() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry23) {
        Sentry23 = require('@sentry/react-native');
    }
    ...
    Sentry23.init({
        ...
        integrations: [
            Sentry23.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException23(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry23.captureException(error);
}

export function captureJSException23(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb23(error, isFatal);
    } else {
        captureException23(error);
    }
}

function captureClientErrorAsBreadcrumb23(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_23 : BREADCRUMB_UNCAUGHT_NON_ERROR_23,
        data: {
    ...
    try {
        Sentry23.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext23 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry23) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext23(database);
        Sentry23.setContext('User-Information', userContext);

        const buildContext = await getBuildTags23(database);
        Sentry23.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext23(database);
        Sentry23.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext23 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext23 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags23 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
