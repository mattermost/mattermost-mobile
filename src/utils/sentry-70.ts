...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_70 = 'uncaught-app-error-70';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_70 = 'uncaught-non-error-70';

let Sentry70: any;
export function initializeSentry70() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry70) {
        Sentry70 = require('@sentry/react-native');
    }
    ...
    Sentry70.init({
        ...
        integrations: [
            Sentry70.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException70(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry70.captureException(error);
}

export function captureJSException70(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb70(error, isFatal);
    } else {
        captureException70(error);
    }
}

function captureClientErrorAsBreadcrumb70(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_70 : BREADCRUMB_UNCAUGHT_NON_ERROR_70,
        data: {
    ...
    try {
        Sentry70.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext70 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry70) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext70(database);
        Sentry70.setContext('User-Information', userContext);

        const buildContext = await getBuildTags70(database);
        Sentry70.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext70(database);
        Sentry70.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext70 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext70 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags70 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
