...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_61 = 'uncaught-app-error-61';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_61 = 'uncaught-non-error-61';

let Sentry61: any;
export function initializeSentry61() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry61) {
        Sentry61 = require('@sentry/react-native');
    }
    ...
    Sentry61.init({
        ...
        integrations: [
            Sentry61.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException61(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry61.captureException(error);
}

export function captureJSException61(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb61(error, isFatal);
    } else {
        captureException61(error);
    }
}

function captureClientErrorAsBreadcrumb61(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_61 : BREADCRUMB_UNCAUGHT_NON_ERROR_61,
        data: {
    ...
    try {
        Sentry61.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext61 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry61) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext61(database);
        Sentry61.setContext('User-Information', userContext);

        const buildContext = await getBuildTags61(database);
        Sentry61.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext61(database);
        Sentry61.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext61 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext61 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags61 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
