...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_35 = 'uncaught-app-error-35';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_35 = 'uncaught-non-error-35';

let Sentry35: any;
export function initializeSentry35() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry35) {
        Sentry35 = require('@sentry/react-native');
    }
    ...
    Sentry35.init({
        ...
        integrations: [
            Sentry35.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException35(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry35.captureException(error);
}

export function captureJSException35(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb35(error, isFatal);
    } else {
        captureException35(error);
    }
}

function captureClientErrorAsBreadcrumb35(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_35 : BREADCRUMB_UNCAUGHT_NON_ERROR_35,
        data: {
    ...
    try {
        Sentry35.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext35 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry35) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext35(database);
        Sentry35.setContext('User-Information', userContext);

        const buildContext = await getBuildTags35(database);
        Sentry35.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext35(database);
        Sentry35.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext35 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext35 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags35 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
