...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_11 = 'uncaught-app-error-11';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_11 = 'uncaught-non-error-11';

let Sentry11: any;
export function initializeSentry11() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry11) {
        Sentry11 = require('@sentry/react-native');
    }
    ...
    Sentry11.init({
        ...
        integrations: [
            Sentry11.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException11(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry11.captureException(error);
}

export function captureJSException11(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb11(error, isFatal);
    } else {
        captureException11(error);
    }
}

function captureClientErrorAsBreadcrumb11(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_11 : BREADCRUMB_UNCAUGHT_NON_ERROR_11,
        data: {
    ...
    try {
        Sentry11.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext11 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry11) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext11(database);
        Sentry11.setContext('User-Information', userContext);

        const buildContext = await getBuildTags11(database);
        Sentry11.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext11(database);
        Sentry11.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext11 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext11 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags11 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
