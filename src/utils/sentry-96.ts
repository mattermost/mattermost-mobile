...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_96 = 'uncaught-app-error-96';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_96 = 'uncaught-non-error-96';

let Sentry96: any;
export function initializeSentry96() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry96) {
        Sentry96 = require('@sentry/react-native');
    }
    ...
    Sentry96.init({
        ...
        integrations: [
            Sentry96.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException96(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry96.captureException(error);
}

export function captureJSException96(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb96(error, isFatal);
    } else {
        captureException96(error);
    }
}

function captureClientErrorAsBreadcrumb96(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_96 : BREADCRUMB_UNCAUGHT_NON_ERROR_96,
        data: {
    ...
    try {
        Sentry96.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext96 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry96) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext96(database);
        Sentry96.setContext('User-Information', userContext);

        const buildContext = await getBuildTags96(database);
        Sentry96.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext96(database);
        Sentry96.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext96 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext96 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags96 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
