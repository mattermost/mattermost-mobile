...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_31 = 'uncaught-app-error-31';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_31 = 'uncaught-non-error-31';

let Sentry31: any;
export function initializeSentry31() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry31) {
        Sentry31 = require('@sentry/react-native');
    }
    ...
    Sentry31.init({
        ...
        integrations: [
            Sentry31.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException31(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry31.captureException(error);
}

export function captureJSException31(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb31(error, isFatal);
    } else {
        captureException31(error);
    }
}

function captureClientErrorAsBreadcrumb31(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_31 : BREADCRUMB_UNCAUGHT_NON_ERROR_31,
        data: {
    ...
    try {
        Sentry31.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext31 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry31) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext31(database);
        Sentry31.setContext('User-Information', userContext);

        const buildContext = await getBuildTags31(database);
        Sentry31.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext31(database);
        Sentry31.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext31 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext31 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags31 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
