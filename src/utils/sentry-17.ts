...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_17 = 'uncaught-app-error-17';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_17 = 'uncaught-non-error-17';

let Sentry17: any;
export function initializeSentry17() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry17) {
        Sentry17 = require('@sentry/react-native');
    }
    ...
    Sentry17.init({
        ...
        integrations: [
            Sentry17.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException17(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry17.captureException(error);
}

export function captureJSException17(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb17(error, isFatal);
    } else {
        captureException17(error);
    }
}

function captureClientErrorAsBreadcrumb17(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_17 : BREADCRUMB_UNCAUGHT_NON_ERROR_17,
        data: {
    ...
    try {
        Sentry17.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext17 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry17) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext17(database);
        Sentry17.setContext('User-Information', userContext);

        const buildContext = await getBuildTags17(database);
        Sentry17.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext17(database);
        Sentry17.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext17 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext17 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags17 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
