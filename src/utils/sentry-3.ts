...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_3 = 'uncaught-app-error-3';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_3 = 'uncaught-non-error-3';

let Sentry3: any;
export function initializeSentry3() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry3) {
        Sentry3 = require('@sentry/react-native');
    }
    ...
    Sentry3.init({
        ...
        integrations: [
            Sentry3.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException3(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
...
export function captureJSException3(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb3(error, isFatal);
    } else {
        captureException3(error);
    }
}

function captureClientErrorAsBreadcrumb3(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_3 : BREADCRUMB_UNCAUGHT_NON_ERROR_3,
        data: {
...
export const addSentryContext3 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry3) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext3(database);
        Sentry3.setContext('User-Information', userContext);

        const buildContext = await getBuildTags3(database);
        Sentry3.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext3(database);
        Sentry3.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext3 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext3 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags3 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
