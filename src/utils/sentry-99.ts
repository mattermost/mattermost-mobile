...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_99 = 'uncaught-app-error-99';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_99 = 'uncaught-non-error-99';

let Sentry99: any;
export function initializeSentry99() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry99) {
        Sentry99 = require('@sentry/react-native');
    }
    ...
    Sentry99.init({
        ...
        integrations: [
            Sentry99.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException99(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry99.captureException(error);
}

export function captureJSException99(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb99(error, isFatal);
    } else {
        captureException99(error);
    }
}

function captureClientErrorAsBreadcrumb99(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_99 : BREADCRUMB_UNCAUGHT_NON_ERROR_99,
        data: {
    ...
    try {
        Sentry99.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext99 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry99) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext99(database);
        Sentry99.setContext('User-Information', userContext);

        const buildContext = await getBuildTags99(database);
        Sentry99.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext99(database);
        Sentry99.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext99 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext99 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags99 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
