...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_58 = 'uncaught-app-error-58';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_58 = 'uncaught-non-error-58';

let Sentry58: any;
export function initializeSentry58() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry58) {
        Sentry58 = require('@sentry/react-native');
    }
    ...
    Sentry58.init({
        ...
        integrations: [
            Sentry58.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException58(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry58.captureException(error);
}

export function captureJSException58(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb58(error, isFatal);
    } else {
        captureException58(error);
    }
}

function captureClientErrorAsBreadcrumb58(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_58 : BREADCRUMB_UNCAUGHT_NON_ERROR_58,
        data: {
    ...
    try {
        Sentry58.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext58 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry58) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext58(database);
        Sentry58.setContext('User-Information', userContext);

        const buildContext = await getBuildTags58(database);
        Sentry58.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext58(database);
        Sentry58.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext58 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext58 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags58 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
