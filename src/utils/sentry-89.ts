...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_89 = 'uncaught-app-error-89';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_89 = 'uncaught-non-error-89';

let Sentry89: any;
export function initializeSentry89() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry89) {
        Sentry89 = require('@sentry/react-native');
    }
    ...
    Sentry89.init({
        ...
        integrations: [
            Sentry89.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException89(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry89.captureException(error);
}

export function captureJSException89(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb89(error, isFatal);
    } else {
        captureException89(error);
    }
}

function captureClientErrorAsBreadcrumb89(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_89 : BREADCRUMB_UNCAUGHT_NON_ERROR_89,
        data: {
    ...
    try {
        Sentry89.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext89 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry89) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext89(database);
        Sentry89.setContext('User-Information', userContext);

        const buildContext = await getBuildTags89(database);
        Sentry89.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext89(database);
        Sentry89.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext89 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext89 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags89 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
