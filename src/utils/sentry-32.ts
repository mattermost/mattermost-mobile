...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_32 = 'uncaught-app-error-32';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_32 = 'uncaught-non-error-32';

let Sentry32: any;
export function initializeSentry32() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry32) {
        Sentry32 = require('@sentry/react-native');
    }
    ...
    Sentry32.init({
        ...
        integrations: [
            Sentry32.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException32(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry32.captureException(error);
}

export function captureJSException32(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb32(error, isFatal);
    } else {
        captureException32(error);
    }
}

function captureClientErrorAsBreadcrumb32(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_32 : BREADCRUMB_UNCAUGHT_NON_ERROR_32,
        data: {
    ...
    try {
        Sentry32.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext32 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry32) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext32(database);
        Sentry32.setContext('User-Information', userContext);

        const buildContext = await getBuildTags32(database);
        Sentry32.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext32(database);
        Sentry32.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext32 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext32 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags32 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
