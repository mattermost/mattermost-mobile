...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_88 = 'uncaught-app-error-88';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_88 = 'uncaught-non-error-88';

let Sentry88: any;
export function initializeSentry88() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry88) {
        Sentry88 = require('@sentry/react-native');
    }
    ...
    Sentry88.init({
        ...
        integrations: [
            Sentry88.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException88(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry88.captureException(error);
}

export function captureJSException88(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb88(error, isFatal);
    } else {
        captureException88(error);
    }
}

function captureClientErrorAsBreadcrumb88(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_88 : BREADCRUMB_UNCAUGHT_NON_ERROR_88,
        data: {
    ...
    try {
        Sentry88.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext88 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry88) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext88(database);
        Sentry88.setContext('User-Information', userContext);

        const buildContext = await getBuildTags88(database);
        Sentry88.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext88(database);
        Sentry88.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext88 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext88 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags88 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
