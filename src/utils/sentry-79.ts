...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_79 = 'uncaught-app-error-79';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_79 = 'uncaught-non-error-79';

let Sentry79: any;
export function initializeSentry79() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry79) {
        Sentry79 = require('@sentry/react-native');
    }
    ...
    Sentry79.init({
        ...
        integrations: [
            Sentry79.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException79(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry79.captureException(error);
}

export function captureJSException79(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb79(error, isFatal);
    } else {
        captureException79(error);
    }
}

function captureClientErrorAsBreadcrumb79(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_79 : BREADCRUMB_UNCAUGHT_NON_ERROR_79,
        data: {
    ...
    try {
        Sentry79.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext79 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry79) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext79(database);
        Sentry79.setContext('User-Information', userContext);

        const buildContext = await getBuildTags79(database);
        Sentry79.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext79(database);
        Sentry79.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext79 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext79 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags79 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
