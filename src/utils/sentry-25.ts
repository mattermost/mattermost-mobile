...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_25 = 'uncaught-app-error-25';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_25 = 'uncaught-non-error-25';

let Sentry25: any;
export function initializeSentry25() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry25) {
        Sentry25 = require('@sentry/react-native');
    }
    ...
    Sentry25.init({
        ...
        integrations: [
            Sentry25.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException25(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry25.captureException(error);
}

export function captureJSException25(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb25(error, isFatal);
    } else {
        captureException25(error);
    }
}

function captureClientErrorAsBreadcrumb25(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_25 : BREADCRUMB_UNCAUGHT_NON_ERROR_25,
        data: {
    ...
    try {
        Sentry25.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext25 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry25) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext25(database);
        Sentry25.setContext('User-Information', userContext);

        const buildContext = await getBuildTags25(database);
        Sentry25.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext25(database);
        Sentry25.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext25 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext25 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags25 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
