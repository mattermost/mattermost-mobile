...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_13 = 'uncaught-app-error-13';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_13 = 'uncaught-non-error-13';

let Sentry13: any;
export function initializeSentry13() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry13) {
        Sentry13 = require('@sentry/react-native');
    }
    ...
    Sentry13.init({
        ...
        integrations: [
            Sentry13.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException13(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry13.captureException(error);
}

export function captureJSException13(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb13(error, isFatal);
    } else {
        captureException13(error);
    }
}

function captureClientErrorAsBreadcrumb13(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_13 : BREADCRUMB_UNCAUGHT_NON_ERROR_13,
        data: {
    ...
    try {
        Sentry13.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext13 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry13) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext13(database);
        Sentry13.setContext('User-Information', userContext);

        const buildContext = await getBuildTags13(database);
        Sentry13.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext13(database);
        Sentry13.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext13 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext13 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags13 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
