...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_24 = 'uncaught-app-error-24';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_24 = 'uncaught-non-error-24';

let Sentry24: any;
export function initializeSentry24() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry24) {
        Sentry24 = require('@sentry/react-native');
    }
    ...
    Sentry24.init({
        ...
        integrations: [
            Sentry24.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException24(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry24.captureException(error);
}

export function captureJSException24(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb24(error, isFatal);
    } else {
        captureException24(error);
    }
}

function captureClientErrorAsBreadcrumb24(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_24 : BREADCRUMB_UNCAUGHT_NON_ERROR_24,
        data: {
    ...
    try {
        Sentry24.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext24 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry24) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext24(database);
        Sentry24.setContext('User-Information', userContext);

        const buildContext = await getBuildTags24(database);
        Sentry24.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext24(database);
        Sentry24.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext24 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext24 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags24 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
