...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_80 = 'uncaught-app-error-80';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_80 = 'uncaught-non-error-80';

let Sentry80: any;
export function initializeSentry80() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry80) {
        Sentry80 = require('@sentry/react-native');
    }
    ...
    Sentry80.init({
        ...
        integrations: [
            Sentry80.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException80(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry80.captureException(error);
}

export function captureJSException80(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb80(error, isFatal);
    } else {
        captureException80(error);
    }
}

function captureClientErrorAsBreadcrumb80(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_80 : BREADCRUMB_UNCAUGHT_NON_ERROR_80,
        data: {
    ...
    try {
        Sentry80.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext80 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry80) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext80(database);
        Sentry80.setContext('User-Information', userContext);

        const buildContext = await getBuildTags80(database);
        Sentry80.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext80(database);
        Sentry80.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext80 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext80 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags80 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
