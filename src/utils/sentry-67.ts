...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_67 = 'uncaught-app-error-67';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_67 = 'uncaught-non-error-67';

let Sentry67: any;
export function initializeSentry67() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry67) {
        Sentry67 = require('@sentry/react-native');
    }
    ...
    Sentry67.init({
        ...
        integrations: [
            Sentry67.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException67(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry67.captureException(error);
}

export function captureJSException67(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb67(error, isFatal);
    } else {
        captureException67(error);
    }
}

function captureClientErrorAsBreadcrumb67(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_67 : BREADCRUMB_UNCAUGHT_NON_ERROR_67,
        data: {
    ...
    try {
        Sentry67.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext67 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry67) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext67(database);
        Sentry67.setContext('User-Information', userContext);

        const buildContext = await getBuildTags67(database);
        Sentry67.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext67(database);
        Sentry67.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext67 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext67 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags67 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
