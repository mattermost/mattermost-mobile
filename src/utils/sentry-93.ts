...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_93 = 'uncaught-app-error-93';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_93 = 'uncaught-non-error-93';

let Sentry93: any;
export function initializeSentry93() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry93) {
        Sentry93 = require('@sentry/react-native');
    }
    ...
    Sentry93.init({
        ...
        integrations: [
            Sentry93.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException93(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry93.captureException(error);
}

export function captureJSException93(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb93(error, isFatal);
    } else {
        captureException93(error);
    }
}

function captureClientErrorAsBreadcrumb93(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_93 : BREADCRUMB_UNCAUGHT_NON_ERROR_93,
        data: {
    ...
    try {
        Sentry93.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext93 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry93) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext93(database);
        Sentry93.setContext('User-Information', userContext);

        const buildContext = await getBuildTags93(database);
        Sentry93.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext93(database);
        Sentry93.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext93 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext93 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags93 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
