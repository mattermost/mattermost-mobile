...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_94 = 'uncaught-app-error-94';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_94 = 'uncaught-non-error-94';

let Sentry94: any;
export function initializeSentry94() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry94) {
        Sentry94 = require('@sentry/react-native');
    }
    ...
    Sentry94.init({
        ...
        integrations: [
            Sentry94.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException94(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry94.captureException(error);
}

export function captureJSException94(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb94(error, isFatal);
    } else {
        captureException94(error);
    }
}

function captureClientErrorAsBreadcrumb94(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_94 : BREADCRUMB_UNCAUGHT_NON_ERROR_94,
        data: {
    ...
    try {
        Sentry94.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext94 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry94) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext94(database);
        Sentry94.setContext('User-Information', userContext);

        const buildContext = await getBuildTags94(database);
        Sentry94.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext94(database);
        Sentry94.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext94 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext94 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags94 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
