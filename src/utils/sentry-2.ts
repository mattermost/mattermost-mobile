...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_2 = 'uncaught-app-error-2';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_2 = 'uncaught-non-error-2';

let Sentry2: any;
export function initializeSentry2() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry2) {
        Sentry2 = require('@sentry/react-native');
    }
    ...
    Sentry2.init({
        ...
        integrations: [
            Sentry2.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException2(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
...
export function captureJSException2(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb2(error, isFatal);
    } else {
        captureException2(error);
    }
}

function captureClientErrorAsBreadcrumb2(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_2 : BREADCRUMB_UNCAUGHT_NON_ERROR_2,
        data: {
    ...
    try {
        Sentry2.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext2 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry2) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext2(database);
        Sentry2.setContext('User-Information', userContext);

        const buildContext = await getBuildTags2(database);
        Sentry2.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext2(database);
        Sentry2.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext2 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext2 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags2 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
