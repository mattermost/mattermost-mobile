...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_57 = 'uncaught-app-error-57';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_57 = 'uncaught-non-error-57';

let Sentry57: any;
export function initializeSentry57() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry57) {
        Sentry57 = require('@sentry/react-native');
    }
    ...
    Sentry57.init({
        ...
        integrations: [
            Sentry57.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException57(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry57.captureException(error);
}

export function captureJSException57(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb57(error, isFatal);
    } else {
        captureException57(error);
    }
}

function captureClientErrorAsBreadcrumb57(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_57 : BREADCRUMB_UNCAUGHT_NON_ERROR_57,
        data: {
    ...
    try {
        Sentry57.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext57 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry57) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext57(database);
        Sentry57.setContext('User-Information', userContext);

        const buildContext = await getBuildTags57(database);
        Sentry57.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext57(database);
        Sentry57.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext57 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext57 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags57 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
