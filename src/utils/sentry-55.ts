...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_55 = 'uncaught-app-error-55';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_55 = 'uncaught-non-error-55';

let Sentry55: any;
export function initializeSentry55() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry55) {
        Sentry55 = require('@sentry/react-native');
    }
    ...
    Sentry55.init({
        ...
        integrations: [
            Sentry55.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException55(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry55.captureException(error);
}

export function captureJSException55(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb55(error, isFatal);
    } else {
        captureException55(error);
    }
}

function captureClientErrorAsBreadcrumb55(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_55 : BREADCRUMB_UNCAUGHT_NON_ERROR_55,
        data: {
    ...
    try {
        Sentry55.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext55 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry55) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext55(database);
        Sentry55.setContext('User-Information', userContext);

        const buildContext = await getBuildTags55(database);
        Sentry55.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext55(database);
        Sentry55.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext55 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext55 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags55 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
