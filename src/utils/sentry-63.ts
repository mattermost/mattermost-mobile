...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_63 = 'uncaught-app-error-63';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_63 = 'uncaught-non-error-63';

let Sentry63: any;
export function initializeSentry63() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry63) {
        Sentry63 = require('@sentry/react-native');
    }
    ...
    Sentry63.init({
        ...
        integrations: [
            Sentry63.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException63(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry63.captureException(error);
}

export function captureJSException63(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb63(error, isFatal);
    } else {
        captureException63(error);
    }
}

function captureClientErrorAsBreadcrumb63(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_63 : BREADCRUMB_UNCAUGHT_NON_ERROR_63,
        data: {
    ...
    try {
        Sentry63.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext63 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry63) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext63(database);
        Sentry63.setContext('User-Information', userContext);

        const buildContext = await getBuildTags63(database);
        Sentry63.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext63(database);
        Sentry63.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext63 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext63 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags63 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
