...
export const BREADCRUMB_UNCAUGHT_APP_ERROR_68 = 'uncaught-app-error-68';
export const BREADCRUMB_UNCAUGHT_NON_ERROR_68 = 'uncaught-non-error-68';

let Sentry68: any;
export function initializeSentry68() {
    if (!Config.SentryEnabled) {
        return;
    }

    if (!Sentry68) {
        Sentry68 = require('@sentry/react-native');
    }
    ...
    Sentry68.init({
        ...
        integrations: [
            Sentry68.reactNativeNavigationIntegration({navigation: Navigation}),
        ],
...
export function captureException68(error: unknown) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    Sentry68.captureException(error);
}

export function captureJSException68(error: unknown, isFatal: boolean) {
    if (!Config.SentryEnabled) {
        return;
    }
    ...
    if (error instanceof ClientError) {
        captureClientErrorAsBreadcrumb68(error, isFatal);
    } else {
        captureException68(error);
    }
}

function captureClientErrorAsBreadcrumb68(error: ClientError, isFatal: boolean) {
    const isAppError = Boolean(error.server_error_id);
    const breadcrumb: Breadcrumb = {
        category: isAppError ? BREADCRUMB_UNCAUGHT_APP_ERROR_68 : BREADCRUMB_UNCAUGHT_NON_ERROR_68,
        data: {
    ...
    try {
        Sentry68.addBreadcrumb(breadcrumb);
    } catch (e) {
        logWarning('Failed to capture breadcrumb of non-error', e);
    }
}

export const addSentryContext68 = async (serverUrl: string) => {
    if (!Config.SentryEnabled || !Sentry68) {
        return;
    }

    try {
        const {database} = DatabaseManager.getServerDatabaseAndOperator(serverUrl);
        const userContext = await getUserContext68(database);
        Sentry68.setContext('User-Information', userContext);

        const buildContext = await getBuildTags68(database);
        Sentry68.setContext('App-Build Information', buildContext);

        const extraContext = await getExtraContext68(database);
        Sentry68.setContext('Server-Information', extraContext);
    } catch (e) {
        logError('addSentryContext for serverUrl ', e);
    }
};

const getUserContext68 = async (database: Database) => {
    const currentUser = {
        id: 'currentUserId',
        locale: 'en',
...
const getExtraContext68 = async (database: Database) => {
    const context = {
        config: {},
        currentChannel: {},
        currentTeam: {},
...
const getBuildTags68 = async (database: Database) => {
    const tags = {
        serverBuildHash: '',
        serverBuildNumber: '',
    };
