...
import {fetchMyCategories, MyCategoriesRequest} from '@actions/remote/category';
...
export const fetchAppEntryData = async (serverUrl: string, initialTeamId: string): Promise<AppEntryData | AppEntryError> => {
    ...
    const promises: [Promise<MyTeamsRequest>, Promise<MyCategoriesRequest | undefined>, Promise<MyChannelsRequest | undefined>, Promise<MyPreferencesRequest>, Promise<MyUserRequest>] = [
        fetchMyTeams(serverUrl, fetchOnly),
        initialTeamId ? fetchMyCategories(serverUrl, initialTeamId, fetchOnly) : Promise.resolve(undefined),
