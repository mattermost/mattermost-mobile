...
export const doPing = async (serverUrl: string, verifyPushProxy: boolean, timeoutInterval = 5000, sharedPassword?: string) => {
    let client: Client;
    try {
        client = await NetworkManager.createClient(serverUrl, undefined, sharedPassword);
    } catch (error) {
        return {error};
    }
