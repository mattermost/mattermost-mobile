...
// so components only react to their own team's sync state — no cross-team noise.
//
// Intentionally ephemeral: resets on app kill so the blob badge is always shown
// on a fresh cold start until threads are synced from the server.
class ThreadsSyncStoreSingleton {
    ...
    markThreadsFetched(serverUrl: string, teamId: string): void {
        this.getOrCreate(serverUrl, teamId).next(true);
    }

    hasThreadsBeenFetched(serverUrl: string, teamId: string): boolean {
...
const ThreadsSyncStore = new ThreadsSyncStoreSingleton();
export default ThreadsSyncStore;
