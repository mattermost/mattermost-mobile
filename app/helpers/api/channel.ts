...
export function publicChannelJoinPrompt(displayName: string, intl: IntlShape): Promise<{join: boolean}> {
    return new Promise((resolve) => {
        Alert.alert(
            intl.formatMessage({
                id: 'permalink.show_dialog_warn.public.title',
                defaultMessage: 'Join channel',
            }),
            intl.formatMessage({
                id: 'permalink.show_dialog_warn.public.description',
                defaultMessage: 'You are about to join {channel}. Are you sure you wish to join this channel?',
            }, {
                channel: displayName,
            }),
            [
