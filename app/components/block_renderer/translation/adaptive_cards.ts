...
function translateAdaptiveCardToggleInput(i: Record<string, unknown>): MmBoolInputBlock | null {
    ...
    const title = ensureString(i.title).trim();
    const explicitLabel = ensureString(i.label).trim();

    // bool_input requires a label; fall back to Adaptive Cards `title` when `label` is omitted.
    const label = explicitLabel || title;
    if (!label) {
        return null;
    }
