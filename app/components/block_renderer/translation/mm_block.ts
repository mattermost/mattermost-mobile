...
const TEXT_INPUT_REQUIRED_KEYS = ['type', 'name'];
const BOOL_INPUT_REQUIRED_KEYS = ['type', 'name', 'label'];
const SELECT_INPUT_REQUIRED_KEYS = ['type', 'name'];
const DATE_INPUT_REQUIRED_KEYS = ['type', 'name'];
const DATETIME_INPUT_REQUIRED_KEYS = ['type', 'name'];
const FILE_INPUT_REQUIRED_KEYS = ['type', 'name'];
...
/** Parse shared form-input label/optional/disabled/placeholder/help_text/onChange. Null if invalid. */
function parseCommonInputFields(raw: Record<string, unknown>): CommonInputFields | null {
    const label = parseOptionalStringField(raw, 'label');
    if (label === null) {
        return null;
    }
...
function translateFileInputBlock(raw: Record<string, unknown>): MmFileInputBlock | null {
    ...
    // Comma-separated string or string[]; FileInputElement.normalizeFileIds accepts both shapes.
    ...
    if (raw.initial_value === undefined) {
        ...
        const asArray = translateStringArray(raw.initial_value);
        if (asArray === null) {
            return null;
        }
        initialValue = asArray?.join(',');
    }
...
function translateBoolInputBlock(raw: Record<string, unknown>): MmBoolInputBlock | null {
    ...
    const common = parseCommonInputFields(raw);
    if (!common?.label?.trim()) {
        return null;
    }

    const initialValue = parseOptionalBooleanField(raw, 'initial_value');
    if (initialValue === null) {
        return null;
    }

    const {label, ...rest} = common;
