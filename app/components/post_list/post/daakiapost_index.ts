...
import DaakiaPost from './daakiapost';
...
const withDaakiaPost = withObservables(
    ...
    ({currentUser, database, isCRTEnabled, post, previousPost, nextPost, location, channelType}: PropsInput) => {
        ...
        const hasReplies = observeHasReplies(database, post);
...
export default React.memo(withDatabase(withSystem(withDaakiaPost(DaakiaPost))));
