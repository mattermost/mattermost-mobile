...
const paddingLeft = (isLandscape, modifier = 0) => {
    return DeviceTypes.IS_IPHONE_X && isLandscape ? {paddingLeft: ViewTypes.IOS_HORIZONTAL_LANDSCAPE} : null;
};

const paddingRight = (isLandscape, modifier = 0) => {
    return DeviceTypes.IS_IPHONE_X && isLandscape ? {paddingRight: ViewTypes.IOS_HORIZONTAL_LANDSCAPE} : null;
};

const marginHorizontal = (isLandscape, modifier = 0) => {
    return DeviceTypes.IS_IPHONE_X && isLandscape ? {marginHorizontal: ViewTypes.IOS_HORIZONTAL_LANDSCAPE} : null;
};
