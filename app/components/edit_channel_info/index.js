...
export default class EditChannelInfo extends PureComponent {
    ...
    canUpdate = (displayName, purpose, header) => {
        const {
            oldDisplayName,
            oldPurpose,
            oldHeader,
    ...
    onDisplayNameChangeText = (displayName) => {
        ...
        if (editing) {
            const {purpose, header} = this.props;
            const canUpdate = this.canUpdate(displayName, purpose, header);
    ...
    onHeaderChangeText = (header) => {
        ...
        if (editing) {
            const {displayName, purpose} = this.props;
            const canUpdate = this.canUpdate(displayName, purpose, header);
            this.enableRightButton(canUpdate);
        }
    };
