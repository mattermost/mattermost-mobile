...
export interface ClientThreadsMix {
    getThreads: (serverVersion: string, userId: string, teamId: string, before?: string, after?: string, pageSize?: number, deleted?: boolean, unread?: boolean, since?: number) => Promise<GetUserThreadsResponse>;
...
const ClientThreads = (superclass: any) => class extends superclass {
    ...
    updateThreadRead = (userId: string, teamId: string, threadId: string, timestamp: number) => {
        const url = `${this.getThreadsRoute(userId, teamId, threadId)}/read/${timestamp}`;
        return this.doFetch(
            url,
            {method: 'put'},
        );
    ...
    updateThreadFollow = (userId: string, teamId: string, threadId: string, state: boolean) => {
        const url = this.getThreadsRoute(userId, teamId, threadId) + '/following';
        return this.doFetch(
            url,
