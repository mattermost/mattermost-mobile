...
describe('formatTime', () => {
    ...
    describe('humanReadable format', () => {
        ...
        it('should format minutes only when hours are zero and seconds are zero', () => {
            expect(formatTime(60, true)).toBe('1m');
            expect(formatTime(120, true)).toBe('2m');
        });

        it('should format hours and minutes when seconds are zero', () => {
            expect(formatTime(3660, true)).toBe('1h 1m');
            expect(formatTime(7320, true)).toBe('2h 2m');
        });

        it('should format hours and seconds when minutes are zero', () => {
            expect(formatTime(3605, true)).toBe('1h 5s');
            expect(formatTime(7205, true)).toBe('2h 5s');
        });
    });
});
