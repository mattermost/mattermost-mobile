...
function AppsFormFileField({
    ...
    // Gates the push-to-parent effect until mount-time hydration has resolved, so the
    // effect cannot clear a pre-populated value before we know which ids are still valid.
    ...
    useDidMount(() => {
        ...
        (async () => {
            const {files, error} = await fetchFilesInfo(serverUrl, ids);
            ...
            if (!isMountedRef.current || hasInteractedRef.current) {
                hydrationSettledRef.current = true;
                return;
            }

            // A hard failure means we could not determine which ids are still valid, so
            // leave the value untouched rather than reporting the files as gone. An
            // empty `files` WITHOUT an error does mean every id was rejected, and must
            // fall through so the stale ids are dropped below — otherwise the field
            // renders empty while still submitting ids the user cannot see or remove.
            if (error) {
                hydrationSettledRef.current = true;
                return;
            }
    ...
    // Keep the parent form's value in sync with the settled entries.
    //
    // `value` is a dependency, and the comparison below is what makes this a sync rather
    // than a one-shot push: the parent re-initialises its values whenever the server
    // returns a refreshed form (a field with refresh=true, or a multiform step), which
    // resets this field to the new element's default. This component is not remounted
    // (same key), so `entries` is unchanged and an entries-only effect never re-fires —
    // leaving the file visible in the UI while the submission carries no file_ids.
    //
    // Assigning through onChange converges: the next run sees ids === value and stops.
    useEffect(() => {
        if (entries.some((e) => e.status === 'uploading') || !hydrationSettledRef.current) {
            return;
        }
        const ids = entries.filter((e) => e.status === 'uploaded' && e.fileId).map((e) => e.fileId!).join(',');
        if (ids !== value) {
            onChangeRef.current(name, ids);
        }
    }, [entries, name, value]);
