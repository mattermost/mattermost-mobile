...
describe('AppsFormFileField', () => {
    ...
    describe('pre-population (hydration from value)', () => {
        ...
        it('clears the value when every pre-populated file is gone', async () => {
            // An empty files array with no error means every requested id was rejected
            // (deleted server-side). The stale ids must be dropped from the parent value:
            // otherwise the field renders empty while submit still carries ids the user
            // can neither see nor remove, and the plugin rejects the submission.
            mockFetchFilesInfo.mockResolvedValue({files: []});

            renderWithIntlAndTheme(
                <AppsFormFileField {...getBaseProps({value: 'deleted-1,deleted-2'})}/>,
            );

            await waitFor(() => {
                expect(mockOnChange).toHaveBeenCalledWith('file_field', '');
            });
        });

        it('keeps the value when hydration fails outright', async () => {
            // A hard failure (client unavailable) also yields an empty files array, but
            // tells us nothing about whether the ids are still valid — so the value must
            // be left alone rather than reported as gone.
            mockFetchFilesInfo.mockResolvedValue({files: [], error: new Error('client unavailable')});

            renderWithIntlAndTheme(
                <AppsFormFileField {...getBaseProps({value: 'still-valid-1'})}/>,
            );

            // Give the hydration promise a turn to settle before asserting absence.
            await act(async () => {
    ...
    describe('upload flow', () => {
        ...
        it('removes a file entry when remove button is pressed', async () => {
            ...
            // Echo the uploaded id back as the value prop. The real parent
            // (AppsFormComponent) is controlled and does exactly this, and the field now
            // syncs against `value` rather than pushing blindly — so leaving the harness
            // uncontrolled would make the field believe the parent is already empty and
            // correctly skip the clearing call asserted below.
            await act(async () => {
                rerender(<AppsFormFileField {...getBaseProps({value: FILE_ID})}/>);
            });
        ...
        it('re-asserts uploaded IDs when the parent resets the value on form refresh', async () => {
            // A field with refresh=true (or a multiform step) makes the server return a
            // refreshed form, and the parent re-runs initValues — resetting this field to
            // the new element's default. The component is NOT remounted (same key), so
            // `entries` is unchanged; an entries-only effect would never re-fire, leaving
            // the file visible in the UI while the submission carries no file_ids.
            const extracted = makeExtractedFile();
            ...
            // The controlled parent accepts the pushed id.
            await act(async () => {
                rerender(<AppsFormFileField {...getBaseProps({value: FILE_ID})}/>);
            });
            ...
            // The refreshed form then resets this field's value to the element default.
            await act(async () => {
                rerender(<AppsFormFileField {...getBaseProps({value: ''})}/>);
            });

            // The still-attached file must be pushed back up, or it is silently dropped
            // from the submission while remaining visible on screen.
            await waitFor(() => {
                expect(mockOnChange).toHaveBeenCalledWith('file_field', FILE_ID);
            });
        });
