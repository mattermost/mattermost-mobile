...
const HOME_DAAKIA_TOTAL_MENTIONS_EVENT = 'home_daakia_total_mentions_event';
...
const unreadsSubscription = (serverUrl: string, {myChannels, settings, threadMentionCount}: UnreadObserverArgs) => {
    ...
    if (unreads) {
        ...
        DeviceEventEmitter.emit(HOME_DAAKIA_TOTAL_MENTIONS_EVENT);
    }
};
...
const HomeDaakia = ({isFocused, theme}: Props) => {
    ...
    useEffect(() => {
        ...
        const totalSubscription = DeviceEventEmitter.addListener(HOME_DAAKIA_TOTAL_MENTIONS_EVENT, totalMentionsEventListener);
        const subscription = subscribeAllServers(serversObserver);
        return () => {
            totalSubscription.remove();
            subscription?.unsubscribe();
            subscriptions.forEach((unreads) => unreads.subscription?.unsubscribe());
            subscriptions.clear();
        };
    }, []);
