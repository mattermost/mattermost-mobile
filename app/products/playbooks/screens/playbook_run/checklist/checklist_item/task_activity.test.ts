...
describe('getTaskActivity', () => {
    ...
    it('should treat a null event list as no events, since the API can send null for an empty timeline', () => {
    ...
    it('should return the checked action, timestamp, and actor for one exact event', () => {
    ...
    it('should return the unchecked action for an open task', () => {
    ...
    it('should keep the action time when no actor event can be matched', () => {
    ...
    ])('should not show activity for untouched or in-progress tasks', (item) => {
        expect(getTaskActivity(item, [makeEvent()])).toBeUndefined();
    });

    it('should return the skip action and actor for a skipped task', () => {
    ...
    ])('should show nothing for a skipped task with no skip event, since state_modified is not the skip time', (item) => {
    ...
    it('should return the restore action for an open task whose event says it was restored', () => {
    ...
    it('should fall back to uncheck for an open task with no matching event, since open cannot distinguish it from a restore', () => {
    ...
    it.each<[ChecklistItemState, string]>([
        ...
        ['skipped', 'check'],
        ['', 'check'],
    ])('should ignore an event whose action the %p state could not have produced (%p)', (state, action) => {
    ...
    ])('should ignore malformed details and events with the wrong action, time, or type', (event) => {
        expect(getTaskActivity(makeItem(), [event])?.actorUserId).toBeUndefined();
    });

    it('should use the unique title match to break a same-millisecond collision', () => {
    ...
    it('should not attribute an actor when a same-millisecond collision remains ambiguous', () => {
    ...
    it('should accept a single action/time match without requiring a title match', () => {
    ...
    describe('item_id matching', () => {
        ...
        it('should prefer an id-carrying event for us over a co-timestamped legacy event', () => {
        ...
        it('should use item_id to resolve a same-millisecond collision that the title cannot', () => {
        ...
        it('should never attribute an event that names a different item', () => {
            const other = withItemId('item-2', {subject_user_id: 'user-2'});

            expect(getTaskActivity(makeItem(), [other])?.actorUserId).toBeUndefined();
        });

        it('should match our legacy event while ignoring an id-carrying event for another item', () => {
        ...
        it('should resolve a markdown title via item_id, which the stripped-title tiebreak cannot', () => {
