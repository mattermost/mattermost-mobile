...
class WebsocketManagerSingleton {
    ...
    public init = async (serverCredentials: ServerCredential[]) => {
        ...
        serverCredentials.forEach(
            ({serverUrl, token, sharedPassword}) => {
                try {
                    DatabaseManager.getServerDatabaseAndOperator(serverUrl);
                    this.createClient(serverUrl, token, sharedPassword);
                } catch (error) {
    ...
    public createClient = (serverUrl: string, bearerToken: string, sharedPassword?: string) => {
        if (this.clients[serverUrl]) {
            this.invalidateClient(serverUrl);
        }

        const client = new WebSocketClient(serverUrl, bearerToken, sharedPassword);
