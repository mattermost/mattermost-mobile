...
// (forwarded on the upstream integration request under `context.form_values`); block `query`
// stays separate (URL params).
//
// Cookie handling:
 ...
 * - `submit`: sends all form input values as typed `form_values` (forwarded under
 *   `context.form_values` on the upstream request). Block `query` remains separate URL query params.
 */
type MmButtonSubtype = 'execute' | 'submit';
 ...
 */
type MmFormFieldProps = {
    name: string;

    /** Field label. Omitted or empty string hides the label (and required/optional markers) in the UI. */
    label?: string;
    help_text?: string;
    optional?: boolean;
    disabled?: boolean;
...
type MmTextInputSubtype = 'text' | 'email' | 'number' | 'password' | 'tel' | 'url';

/** Single- or multi-line text field. Dialog `text` / `textarea` both map here (`multiline` for textarea). */
type MmTextInputBlock = MmFormFieldProps & {
    type: 'text_input';
...
/** Checkbox / toggle. Dialog `bool`. Requires a non-empty `label` (row title). */
type MmBoolInputBlock = MmFormFieldProps & {
    type: 'bool_input';

    /** Required row title shown beside the switch. */
    label: string;

    /** Secondary hint under the label (dialog `placeholder`) when `help_text` is omitted. */
    placeholder?: string;
 ...
 */
type MmSelectInputStyle = 'compact' | 'expanded';

/** Grouped options for form `select` (Block Kit `option_groups` parity). */
type MmSelectOptionGroup = {
    label: string;
    options: MmStaticSelectOption[];
};
 ...
 */
type MmSelectInputBlock = MmFormFieldProps & {
    type: 'select';
    placeholder?: string;
...
type MmDateInputBlock = MmFormFieldProps & {
    type: 'date_input';
    placeholder?: string;
    initial_value?: string;
...
type MmDateTimeInputBlock = MmFormFieldProps & {
    type: 'datetime_input';
    placeholder?: string;
    initial_value?: string;
    datetime_config?: MmDateTimeConfig;
...
type MmFileInputBlock = MmFormFieldProps & {
    type: 'file_input';
    placeholder?: string;
