...
import {env, required, MAESTRO_ENV_FILE} from '../utils/env';
...
function writeEnvFile(vars: Record<string, string>): void {
    const lines = Object.entries(vars).
        map(([k, v]) => `export ${k}="${v}"`).
        join('\n');
    fs.writeFileSync(MAESTRO_ENV_FILE, `#!/bin/bash\n# Auto-generated by maestro/fixtures/seed_file_preview.ts — do not edit\n${lines}\n`);
    console.log(`[seed_file_preview] Wrote env vars to ${MAESTRO_ENV_FILE}`);
}
...
async function main(): Promise<void> {
    ...
    console.log(`  source ${MAESTRO_ENV_FILE} && maestro test maestro/flows/channels/file_type_preview.yml \\`);
