...
Maestro flows run against Matterwick cloud servers (`SITE_1_URL`, `SITE_2_URL`, `SITE_3_URL`). CI provisions via `detox/provision_server.js` + `maestro/fixtures/seed.ts`.
...
| Per-channel calls enable | `maestro/fixtures/seed.ts` | Cloud servers may not auto-enable per channel |
...
Local parity: [README.md § Official local workflow](./README.md#official-local-workflow) and `./maestro/scripts/setup_local.sh`.
...
```bash
cd maestro && npm run validate-headers
```
...
For CI parity, use `maestro/scripts/run_ci_batches.sh` (batch mode) with the CI-built `.app`/APK — not isolated single-flow runs only.
