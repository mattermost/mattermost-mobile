...
**Native build prerequisites:** The E2EE module uses a Rust bridge. Installing the app now requires Rust 1.90.0 with `cargo-ndk` and the iOS/Android Rust targets (`rustup target add aarch64-apple-ios aarch64-apple-ios-sim aarch64-linux-android armv7-linux-androideabi x86_64-linux-android i686-linux-android`). The install script checks for these tools and exits early with guidance if they are missing.
