...
**Canonical copy:** This file and the `maestro/` directory were introduced in [mattermost-mobile#9739](https://github.com/mattermost/mattermost-mobile/pull/9739). If your branch does not include them yet, use that PR as the reference when setting up Cloud Agents.

## Cursor Cloud specific instructions

This is a React Native mobile app (Mattermost Mobile). On Linux Cloud Agent VMs, iOS builds and the iOS simulator are unavailable (macOS-only). Some Cloud Agent images include a software-rendered **Android** emulator (`Pixel_7` AVD, no KVM); when it is missing, development work focuses on JS/TS: linting, type checking, tests, and Metro bundler.
...
# Test server credentials: set in Cursor Secrets as MM_TEST_SERVER_URL, MM_TEST_USER_NAME,
# MM_TEST_PASSWORD — pass into Maestro with -e (do not put real passwords in YAML).
maestro test maestro/login_flow.yaml \
  -e SERVER_URL="$MM_TEST_SERVER_URL" \
  -e SERVER_NAME="testing" \
  -e USERNAME="$MM_TEST_USER_NAME" \
  -e PASSWORD="$MM_TEST_PASSWORD" \
  --debug-output ./maestro-debug
```

For posting a message after login, use `maestro/post_message_flow.yaml` with the same `-e` arguments.
...
Pass `SERVER_URL`, `USERNAME`, and `PASSWORD` from the parent flow using Maestro `-e` (see Quick start). Do not commit production credentials in YAML.
...
- **Metro bundler** binds to `127.0.0.1:8081`. Verify with `curl http://127.0.0.1:8081/status`. On some low-memory Cloud Agent hosts, the default transformer worker pool can return HTTP 500 (`Jest worker … child process exceptions`); if that happens, start with `npx react-native start --reset-cache --max-workers 1` and optionally `NODE_OPTIONS=--max-old-space-size=8192`.
...
- **Android emulator is very slow** without KVM — budget extra time for boot, install, and app initialization. The app may appear stuck on the splash logo for several minutes before the UI renders. After `clearState`, the first dev bundle download + JS mount on a software emulator often needs **30–60 minutes** before server screen `testID`s appear; use `extendedWaitUntil` timeouts accordingly in `maestro/shared/login.yaml`.
